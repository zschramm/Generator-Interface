...
// Resistive Pressure Sender
FloatProducer* ConnectPressureSender(Adafruit_ADS1115* ads1115,
                                     int channel,
                                     String name,
                                     int zero_pres_resistance,
                                     int max_press,
                                     int max_press_resistance) {
  ...
  snprintf(config_path, sizeof(config_path), "%s Pressure (PSI)",
           name.c_str());
  ...
  snprintf(meta_display_name, sizeof(meta_display_name), "Resistance %s",
           name.c_str());
  snprintf(meta_description, sizeof(meta_description),
           "Measured %s sender resistance", name.c_str());

  snprintf(config_path, sizeof(config_path), "/Tank %s/Level Curve",
           name.c_str());
  auto pressure = (new CurveInterpolator(nullptr, config_path))
                        ->set_input_title("Sender Resistance (ohms)")
                        ->set_output_title("Pressure (PSI)");
  sender_resistance->connect_to(pressure);

  pressure->clear_samples();
  pressure->add_sample(CurveInterpolator::Sample(10000, max_press));
  pressure->add_sample(CurveInterpolator::Sample(max_press_resistance, max_press));
  pressure->add_sample(CurveInterpolator::Sample(zero_pres_resistance, 0));
